...
class WidgetP2PList(val gui: GuiAdvancedMemoryCard,
                    private val infos: InfoList,
                    private var x: Int, private var y: Int,
                    private val selectedInfo: KProperty0<InfoWrapper?>,
                    val mode: () -> BetterMemoryCardModes) : ObjectSelectionList<WidgetP2PList.Companion.Entry>(Minecraft.getInstance(),
    254, 0, x, y,
    ...
    fun resize(scale: GuiScale, availableHeight: Int) {
        ...
        for (i in 0 until scale.size(availableHeight)) {
            val widget = Entry(WidgetP2PDevice(i, selectedInfo, mode,
                { infos.filtered.getOrNull(i) },
                gui.col, x, y + i * (P2PEntryConstants.HEIGHT + 1)))
            addEntry(widget)
        }
    }
