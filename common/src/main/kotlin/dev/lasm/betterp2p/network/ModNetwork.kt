...
object ModNetwork {
    ...
    fun <T: IMessage> decoder(factory: () -> T) = { buf: FriendlyByteBuf -> factory().also { it.fromBytes(buf) } }

    fun registerNetwork() {
        channel.register(S2COpenGui::class.java, encoder, decoder(::S2COpenGui), ClientOpenGuiHandler)

        channel.register(S2CUpdateP2P::class.java, encoder, decoder(::S2CUpdateP2P), ClientUpdateP2PHandler)

        channel.register(C2SLinkP2P::class.java, encoder, decoder(::C2SLinkP2P), ServerLinkP2PHandler)
        channel.register(C2SCloseGui::class.java, encoder, decoder(::C2SCloseGui), ServerCloseGuiHandler)
        channel.register(C2SUpdateMemoryInfo::class.java, encoder, decoder(::C2SUpdateMemoryInfo), ServerUpdateMemoryInfoHandler)

        channel.register(C2SRenameP2P::class.java, encoder, decoder(::C2SRenameP2P), ServerRenameP2PTunnelHandler)
        channel.register(C2SRefreshP2PList::class.java, encoder, decoder(::C2SRefreshP2PList), ServerRefreshP2PListHandler)
        channel.register(C2SUnlinkP2P::class.java, encoder, decoder(::C2SUnlinkP2P), ServerUnlinkP2PHandler)
        channel.register(C2STypeChange::class.java, encoder, decoder(::C2STypeChange), ServerTypeChangeHandler)
    ...
    fun requestP2PList(player: Player, type: Int) {
        synchronized(playerState) {
            ...
            if (playerState.updateReady + NETWORK_CD < System.currentTimeMillis()) {
                channel.sendToPlayer(player as ServerPlayer, S2CUpdateP2P(cache.retrieveP2PList(), true))
